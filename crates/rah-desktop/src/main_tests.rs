...
use super::host_invocation::{
    BranchReview, CoordinatorState, EmptyHostRequest, HostConfirmRequest, HostInvocationDescriptor,
    HostInvocationKind, HostInvocationReview, HostPrepareBranchRequest,
    HostPrepareCreateFileRequest, HostPrepareDeleteFileRequest,
...
use super::{
    ActivationOutcome, ActivationTestHook, ActivityEvent, ActivityResult, AuthorizationTestHook,
    BranchActivityClassification, CancelRecoveryOutcome, ChatEvent, ChatState,
    CloseRepositoryError, CloseRepositoryRequest, CodexExecutableSourcePresentation,
    ...
    DesktopCommitIdentity, DesktopConversationState, DesktopModelProvider, DesktopModelSelection,
    DesktopModelState, DesktopRepository, DesktopToolComposition, FrontendError,
    GracefulCancelOutcome, HardShutdownOutcome, HostActivityEvent, HostActivityState,
    HostInvocationCoordinator, HostInvocationUnavailableReason, IndexEffectTestHook,
    LlamaCppReadinessProbe, MAX_CONVERSATION_REPLAY_BYTES, MAX_CONVERSATION_REPLAY_MESSAGES,
    MAX_PROMPT_BYTES, ModelConfigurationPresentation, MultiFileResultClassification,
    NEUTRAL_WORKSPACE_DIRECTORY, PendingConnectedPublication, Persistence, Preferences,
    PreferencesWarning, PreparedDeleteFileResponse, PreparedHostInvocation, PreparedHostPayload,
    ProviderEndpoint, ProviderEndpointInput, ProviderEndpointPresentation,
    ProviderPublicationRejectionReason, ProviderScheme, READINESS_BODY_LIMIT,
...
use rah_tools::{
    ...
    RepositoryMultiFileEditTextReplacement, RepositoryPatchResultClassification, Tool, ToolContext,
    ToolRegistry, authorize_tool_dispatch, authorized_tool_dispatch,
...
fn desktop_connect_preparation_keeps_equivalent_auto_and_override_inputs_identical() {
    ...
    let certified_executable =
        OsString::from(r"C:\Users\morefunfun11\AppData\Local\codex-baselines\0.149.0\codex.exe");
    let cases = [
        (
    ...
    let observed = cases.map(|(name, selection, expected_source)| {
        let prepared = prepare_codex_connection(move || Ok(selection), CodexModelConfig::Inherit)
...
impl TestRepository {
    fn new() -> Self {
        ...
        let root =
            std::env::temp_dir().join(format!("rah-desktop-tool-registry-{timestamp}-{sequence}"));
    ...
    fn deletion_repository(&self) -> DesktopRepository {
        ...
        DesktopRepository::new_with_authorities(&git, &self.0, None, Some(authority), None, None)
    ...
    fn directory_repository(&self) -> DesktopRepository {
        ...
        DesktopRepository::new_with_authorities(&git, &self.0, Some(authority), None, None, None)
    ...
    fn rename_repository(&self) -> DesktopRepository {
        ...
        DesktopRepository::new_with_authorities(&git, &self.0, None, None, Some(authority), None)
    ...
    fn branch_repository(&self) -> DesktopRepository {
        ...
        DesktopRepository::new_with_authorities(&git, &self.0, None, None, None, Some(authority))
...
async fn windows_live_desktop_hostexplicit_create_file() -> Result<(), String> {
    ...
    let git =
        selected_git_executable().map_err(|error| format!("Git discovery failed: {error:?}"))?;
    ...
    let before_generations = current_host_generation_tuple(app.state::<DesktopAppState>().inner());
    ...
    let prepare_root_unchanged = live_directory_entries(&fixture.0)? == before_directory_entries;
    let prepare_parent_unchanged = live_directory_entries(&parent_path)? == before_parent_entries;
    ...
    let actual_bytes =
        fs::read(&target_path).map_err(|error| format!("created target read failed: {error}"))?;
    ...
    if current_host_generation_tuple(app.state::<DesktopAppState>().inner()) != before_generations
        || app.state::<DesktopAppState>().persistence_namespace() != before_namespace
        || serde_json::to_value(
            app.state::<DesktopAppState>()
                .persistence
        ...
        .map_err(|error| format!("terminal conversation serialization failed: {error}"))?
            != before_conversation
    {
        return Err("terminal refresh changed currentness or conversation persistence".to_owned());
    }
...
async fn windows_live_desktop_hostexplicit_delete_file() -> Result<(), String> {
    ...
    let before_target_bytes =
        fs::read(&target_path).map_err(|error| format!("target baseline read failed: {error}"))?;
    ...
    let ineligible_was_eligible = connected_snapshot.effective_tools.iter().any(|tool| {
        ineligible_names.contains(&tool.public_tool_name.as_str()) && tool.host_invocation.eligible
    });
    let external_effective = connected_snapshot
        .effective_tools
        .iter()
    ...
    let before_index = fs::read(fixture.0.join(".git").join("index"))
        .map_err(|error| format!("Git index baseline read failed: {error}"))?;
    let before_cached_binary = live_git_text(&git, &fixture.0, &["diff", "--cached", "--binary"])?;
    let before_git = live_git_state(&git, &fixture.0, "__rah_no_excluded_branch__")?;
    if !before_git
        .status
        .lines()
    ...
    let prepare_cached_unchanged =
        live_git_text(&git, &fixture.0, &["diff", "--cached", "--binary"])? == before_cached_binary;
    let prepare_root_unchanged = live_directory_entries(&fixture.0)? == before_directory_entries;
    let prepare_parent_unchanged = live_directory_entries(&parent_path)? == before_parent_entries;
    let prepare_conversation_unchanged = serde_json::to_value(
        app.state::<DesktopAppState>()
    ...
    if !prepare_index_unchanged
        ...
        || !prepare_root_unchanged
        || !prepare_parent_unchanged
        || fs::read(&target_path).map_err(|error| format!("Prepare target read failed: {error}"))?
            != before_target_bytes
    ...
    {
        return Err("Confirm was not exactly one Tool and one native delete attempt".to_owned());
    }
    ...
    let after_git = live_git_state(&git, &fixture.0, "__rah_no_excluded_branch__")?;
    let after_cached_binary = live_git_text(&git, &fixture.0, &["diff", "--cached", "--binary"])?;
...
async fn observed_single_target_stage_and_unstage_refresh_and_consume_selectors() {
    ...
    let staged = repository_index_action(&state, stage.clone(), RepositoryIndexActionKind::Stage)
        .await
...
async fn old_repository_selector_cannot_resolve_to_new_repository_action() {
    ...
    let result = repository_index_action(&state, new_action_id, RepositoryIndexActionKind::Stage)
        .await
...
async fn task_126_host_probe_uses_desktop_repository_runtime_construction() {
    ...
    let repository =
        DesktopRepository::new(&git, &root).expect("current directory is a selected repository");
...
fn live_git_state(git: &Path, root: &Path, excluded_branch: &str) -> Result<LiveGitState, String> {
...
async fn windows_live_desktop_hostexplicit_rename_file() -> Result<(), String> {
    let codex_selection =
        resolve_codex_executable().map_err(|error| format!("Codex discovery failed: {error:?}"))?;
    if !matches!(
    ...
    let before_source_bytes =
        fs::read(&source_path).map_err(|error| format!("source baseline read failed: {error}"))?;
    ...
    if confirmed.invocation_id != activity_id || commit_control.has_pending_authorization().await {
        return Err(
    ...
    if final_coordinator != CoordinatorState::Idle
        ...
        || live_test_rename_file_native_attempts(&fixture.0) != 1
    {
        return Err("HostExplicit rename left coordinator activity or replay evidence".to_owned());
    }
    ...
    if !shutdown_connection || !shutdown_provider {
        return Err("Desktop shutdown did not complete the existing Codex reaping path".to_owned());
    }
    if startup_activation_snapshot() != StartupActivationCounters::default() {
        return Err("live certification startup counters were not zero after shutdown".to_owned());
    }
...
fn live_patch_temporary_count(root: &Path) -> Result<usize, String> {
    fs::read_dir(root)
        ...
        .try_fold(0, |count, entry| {
            ...
            let is_patch_temporary = name
                .to_str()
                .is_some_and(|name| name.starts_with(".rah-repo-patch-") && name.ends_with(".tmp"));
...
async fn windows_live_desktop_explicit_host_tool_invocation() -> Result<(), String> {
    ...
    {
        return Err("repo.create-branch Effective Authority classification was wrong".to_owned());
    }
    ...
    let generations_before_read =
        current_host_generation_tuple(app.state::<DesktopAppState>().inner());
    let conversation_namespace_before_read = app.state::<DesktopAppState>().persistence_namespace();
    ...
    if read_output.is_error || read_output.content.len() != 1 {
        return Err("repo.status did not return one successful ToolOutput content item".to_owned());
    }
    ...
    if staged.is_none() || unstaged.is_none() {
        return Err("repo.status did not reflect the fresh staged and unstaged fixture".to_owned());
    }
    ...
    {
        return Err("reviewed authorization was not pending before branch preparation".to_owned());
    }
    ...
    {
        return Err("branch prepare did not remain zero effect with the exact review".to_owned());
    }
    if app
        .state::<DesktopAppState>()
        .host_invocation
    ...
    if prepared.review.branch != branch_name
        || get_effective_authority_snapshot(app.state()).status != SnapshotStatus::ConnectedCurrent
        || current_host_generation_tuple(app.state::<DesktopAppState>().inner())
            != branch_tool_generations
        || app.state::<DesktopAppState>().persistence_namespace() != branch_conversation_namespace
    ...
    {
        return Err("branch preparation changed currentness or advertised eligibility".to_owned());
    }
    ...
    if confirmed.invocation_id.is_empty() {
        return Err("branch HostExplicit confirmation returned an empty invocation ID".to_owned());
    }
    ...
    branch_output_marker =
        serde_json::to_string(&branch_output).unwrap_or_else(|_| "serialization-failed".to_owned());
    if !matches!(
    ...
    if !after_branch_tool.host_invocation.eligible
        ...
        || after_branch_tool.authority_category != AuthorityCategory::RepositoryLocalBranchCreation
...
async fn windows_live_desktop_hostexplicit_repo_patch() -> Result<(), String> {
    ...
    let multi_file_tool = find_tool("repo.edit-files")
        .ok_or_else(|| "repo.edit-files was not in the current Effective Authority".to_owned())?;
    ...
    let after_prepare_target = fs::read(&target).map_err(|_| "target read failed".to_owned())?;
    ...
    if after_prepare_target != before_target
        ...
        || fs::read(&sentinel).map_err(|_| "sentinel read failed".to_owned())? != before_sentinel
        || current_host_generation_tuple(app.state::<DesktopAppState>().inner())
    ...
    let staged_names = match live_git_text(&git, &fixture.0, &["diff", "--cached", "--name-only"]) {
        Ok(names) => names,
        Err(error) => fail_after_patch_start!(error),
    };
...
async fn windows_live_desktop_hostexplicit_multi_file_edit() -> Result<(), String> {
    ...
    if before_generations[0] == 0 || before_namespace.is_empty() {
        shutdown_live_state(app.state::<DesktopAppState>().inner()).await;
        return Err("connected-current generation or persistence baseline was empty".to_owned());
    }
    reset_startup_activation_counters();
    ...
    {
        shutdown_live_state(app.state::<DesktopAppState>().inner()).await;
        return Err("Prepare review semantics or protected non-effects were incomplete".to_owned());
    }
        ...
        || require_host_event(&events[2], "tool_completed", "repo.edit-files").is_err()
    {
        shutdown_live_state(app.state::<DesktopAppState>().inner()).await;
        std::mem::forget(fixture);
        return Err("HostExplicit lifecycle was not exactly prepared/started/completed".to_owned());
    }
    if events
        .iter()
    ...
    let after_targets = target_paths
        ...
        .map(|path| {
            let bytes = fs::read(fixture.0.join(path))
                .map_err(|error| format!("post-effect target read failed for {path}: {error}"))?;
            let expected = specifications
                .iter()
                .find(|(candidate, _, _, _)| candidate == path)
    ...
    let (refreshed_workflow_is_descriptive, coordinator_is_idle, chat_is_idle, active_chat_is_idle) = {
        ...
        || get_effective_authority_snapshot(app.state()).status != SnapshotStatus::ConnectedCurrent
    {
        shutdown_live_state(app.state::<DesktopAppState>().inner()).await;
        std::mem::forget(fixture);
        return Err("refresh or terminal Desktop state was not current and descriptive".to_owned());
    }
    let duplicate = host_confirm_tool_invocation(
        HostConfirmRequest {
            ticket_id: prepared.ticket_id.clone(),
...
async fn windows_live_desktop_repo_create_branch() -> Result<(), String> {
    ...
    let composed_registry = desktop_tool_registry(Some(&selected), Some(Arc::clone(&commit_tool)))
    ...
    *state
    ...
    {
        return Err("branch effect changed Desktop currentness or conversation state".to_owned());
    }
...
async fn windows_live_desktop_repo_create_branch_host_driven() -> Result<(), String> {
    ...
    *state
    ...
    if activity.invalidate_review || activity.refresh_reason.is_some() {
        ...
        std::mem::forget(fixture);
        return Err("safe host branch activity invalidated review or requested refresh".to_owned());
    }
        ...
        || !providers_absent
    {
        ...
        std::mem::forget(fixture);
        return Err("host-driven branch effect changed protected Desktop or Git state".to_owned());
    }
...
async fn task360_linked_activation_rejects_gitfile_change_at_publication_barrier() {
    ...
    let activation =
        tokio::spawn(async move { activate_admitted_member(&activation_state, member_b).await });
    reached
        .recv()
        .await
...
async fn task_318_membership_presentation_and_selector_route_are_active_only() {
    ...
    let serialized = serde_json::to_string(&listed).expect("membership presentation serializes");
    ...
    let registry_b =
        desktop_tool_registry(Some(&active_b), None).expect("fresh B repository registry builds");
...
fn task_341_membership_remove_changes_generation_only_after_success() {
    ...
    let member_b = admit_repository(&state, &git, &repository_b.0).expect("admit B");
    {
        ...
        let unknown =
            RepositoryMemberId::parse_selector(&format!("m{}-999999", member_a.as_debug_tuple().0))
...
async fn task_341_removal_preserves_remembered_candidate_bytes() {
    ...
    let catalog = state
        ...
        .add_candidate(
            ...
            Some(
                super::remembered_workspace::RememberedLocationHint::parse(repository_b.0.clone())
    ...
    *state
    ...
    if removed.outcome != RepositoryMemberRemovalOutcomePresentation::Removed
        ...
        || state
            ...
            .is_none()
    {
        return Err("inactive B removal did not produce the required linearized result".to_owned());
    }
    if *state
        .repository_generation
        .lock()
    ...
    if fresh_b == member_b
        || repository_membership_presentation(&state).active_member_id != Some(member_a.selector())
...
async fn task_321_i_real_stage_reservation_rejects_real_connect_publication() {
    ...
    let selected = state.repository.lock().unwrap().clone().unwrap();
    let registry =
        desktop_tool_registry(Some(&selected), None).expect("real Desktop registry should compose");
    let composition =
        desktop_tool_composition_from_registry(Arc::clone(&registry), Some(&selected), false, &[])
...
async fn task349_close_withdraws_active_state_and_preserves_membership_and_persistence() {
    ...
    assert!(
        repository_membership_presentation(&state)
            .members
            .iter()
            .any(|member| RepositoryMemberId::parse_selector(&member.member_id) == Some(member_b))
    );
}
...
async fn task351_close_waits_for_chat_terminal_publication_to_finish() {
    ...
    assert!(state.register_chat_session(chat_generation, Arc::clone(&runtime), session_id.clone()));
    let repository_generation = *state.repository_generation.lock().unwrap();
...
async fn task351_workflow_selector_generations_fail_closed_on_exhaustion() {
    ...
    let action_overflow = super::publish_repository_workflow(&state, make_prepared(vec![action()]));
    {
        let workflow = state.repository_workflow.lock().unwrap();
...
async fn task349_close_for_a_never_closes_a_newly_active_b() {
    ...
    assert_eq!(
        super::close_repository_transition(&state, close_request(member_a, captured_generation),),
        Err(super::CloseRepositoryError::ActiveChanged)
    );
    assert_eq!(
        state.workspace_membership.lock().unwrap().active_member(),
...
async fn task349_close_preserves_runtime_model_and_repository_effect_owners() {
    ...
    *state.chat.lock().unwrap() = ChatState::Idle;
    ...
    let (reservation, _) =
        begin_repository_index_effect(&state, &stage_action_id, RepositoryIndexActionKind::Stage)
    ...
    assert_eq!(*state.repository_generation.lock().unwrap(), generation + 1);
    assert_eq!(
        fs::read(repository_a.0.join(".git").join("index")).expect("Git index remains readable"),
        index_before
    );
    assert_eq!(
...
async fn task349_close_suppresses_late_repository_workflow_refresh_publication() {
    ...
    let refresh = tokio::spawn(async move { refresh_repository_workflow(&refresh_state).await });
    reached
        .recv()
        .await
...
fn branch_authority_is_stored_only_when_resources_match() {
    ...
    assert!(
        DesktopRepository::new_with_authorities(&git, &other.0, None, None, None, Some(authority),)
            .is_err()
    );
}
...
fn branch_effective_authority_is_host_classified_and_unavailable_paths_are_closed() {
    ...
    let unavailable = no_repository
        .unavailable
        .iter()
        .find(|entry| entry.public_tool_name.as_deref() == Some(REPOSITORY_CREATE_BRANCH_TOOL_NAME))
...
async fn production_patch_prepare_finalizes_a_real_ticket_without_effect() {
    ...
    let before_temporary_count =
        live_patch_temporary_count(&fixture.0).expect("temporary artifacts should be observable");
...
fn multi_file_malformed_source_is_never_retained_as_generic_activity() {
    ...
    let classification = classify_repository_multi_file_output(&output, &["safe.txt".to_owned()]);
...
fn create_file_result_classification_is_strict_and_redacted() {
    ...
    assert_eq!(
        super::create_file_host_terminal_state(CreateFileResultClassification::WriteFailedKnown),
...
async fn deletion_hostexplicit_proof_coupling_requires_independent_postcondition() {
    ...
    fs::write(fixture.0.join("tracked.txt"), bytes).expect("replacement should recreate target");
    let proof = (preparation, Arc::clone(&preparer));
    assert_eq!(
        classify_repository_delete_file_result(
            &output("known_no_effect", true),
...
async fn deletion_hostexplicit_success_dispatches_once_and_invalidates_commit_review() {
    ...
    require_host_event(&events[2], "tool_completed", "repo.delete-file").expect("terminal event");
    assert_eq!(
...
async fn desktop_delete_file_requires_authority_and_rejects_stale_preimage_without_replay() {
    ...
    let repository = fixture.deletion_repository();
    let registry =
        desktop_tool_registry(Some(&repository), None).expect("authorized registry should build");
...
async fn desktop_registry_commit_requires_its_paired_control_and_is_one_shot() {
    ...
    let committed = registry
        ...
        .await
        .unwrap();
    let presentation = commit_activity_presentation(&committed).expect("sanitized verified result");
...
fn no_repository_uses_an_app_owned_neutral_workspace_not_its_storage_root() {
    ...
    let canonical_storage = fs::canonicalize(&storage.0).expect("storage root should canonicalize");
...
async fn selected_repository_registry_has_only_the_intended_bounded_tools() {
    ...
    let repository = fixture.desktop_repository();
    let registry = desktop_tool_registry(Some(&repository), None).expect("registry should build");
    let definitions = registry.definitions();
...
fn started_branch_without_finish_is_an_uncertain_repository_effect() {
    ...
    let mut calls = HashMap::new();
    activity_event_with_composition(&requested, &mut calls, &empty_composition_metadata(), true)
        .expect("requested activity");
    let started =
        activity_event_with_composition(&started, &mut calls, &empty_composition_metadata(), true)
...
fn started_branch_without_repository_does_not_manufacture_refresh_state() {
    ...
    let mut calls = HashMap::new();
    activity_event_with_composition(&requested, &mut calls, &empty_composition_metadata(), false)
        .expect("requested activity");
    let started =
        activity_event_with_composition(&started, &mut calls, &empty_composition_metadata(), false)
            .expect("started activity");
    assert!(!started.invalidate_review);
    assert_eq!(started.refresh_reason, None);
    assert!(uncertain_repository_effect_pending(&calls));
...
fn llama_adapter_handoff_uses_the_synthesized_endpoint_without_credentials() {
    ...
    let CodexModelConfig::Explicit(config) = selection.codex_model_config().expect("config") else {
...
fn task_333_catalog_actions_are_durable_ordered_and_privacy_safe() {
    ...
    let durable = super::remembered_workspace::RememberedWorkspaceStore::open(storage.0.clone())
        .expect("remembered store opens")
        .load()
...
fn task_334_reveal_is_explicit_exact_and_authority_free() {
    ...
    let candidate = add_remembered_candidate(&state, "Private repository", Some(sentinel.clone()));
...
async fn task_333_catalog_identity_is_not_process_member_identity_and_isolated() {
    ...
    let process_b = DesktopAppState::new(storage.0.clone());
    let member_b_fresh = super::admit_remembered_candidate(&process_b, candidate.clone()).unwrap();
...
fn startup_remembers_profile_path_without_selecting_or_activating_it() {
    use super::desktop_preferences::{RememberedTrustedProfilePath, clear_test_state, test_lock};
    let _guard = test_lock()
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
    let storage = TestRepository::new();
        ...
        match observed.stream {
            Task324EventStream::Chat => {
                ...
                match kind {
                    ...
                    "completed" => {
                        if !self.chat_started {
                            return Err("failure_stage=chat_completed_before_started".to_owned());
                        }
...
fn task_324_d_process_census(executable: &Path) -> Result<Vec<Task324DProcessObservation>, String> {
    let output = Command::new("powershell")
            .args([
                "-NoProfile",
                "-NonInteractive",
        ...
        Value::Array(items) => items
            ...
            .map(|item| {
                serde_json::from_value(item)
                    .map_err(|error| format!("sanitized process observation was invalid: {error}"))
            })
            .collect(),
...
async fn task_324_d_windows_codex_app_server_ownership_evidence() -> Result<(), String> {
    ...
    {
        return Err(
            "set RAH_RUN_V026_CODEX_OWNERSHIP_TRIAGE=1 for the Windows ownership triage".to_owned(),
        );
    }
    let git = selected_git_executable()
        .map_err(|error| format!("native Git discovery failed: {error:?}"))?;
...
async fn task_324_c_windows_host_driven_two_repository_live_certification() -> Result<(), String> {
    ...
    if !git_metadata.file_type().is_file() || git.extension() != Some(std::ffi::OsStr::new("exe")) {
    ...
    if state
        ...
        || active_a.root
            != fixture
                ...
                .map_err(|error| error.to_string())?
    {
        return Err("A active membership presentation or repository binding was wrong".to_owned());
    }
    ...
    let new_codex_processes_a =
        task_324_d_new_app_server_process_ids(&codex_processes_before_a, &codex_processes_after_a);
    ...
    let process_a = codex_processes_after_a
        ...
        .ok_or_else(|| "A Connect candidate was absent from the post-Connect census".to_owned())?;
    ...
    {
        return Err("A-to-B activation did not publish exactly one fresh generation".to_owned());
    }
    ...
    {
        return Err("B active registry was not the same ordinary non-union namespace".to_owned());
    }
    ...
    let new_codex_processes_b =
        task_324_d_new_app_server_process_ids(&codex_processes_before_b, &codex_processes_after_b);
    ...
    let process_b = codex_processes_after_b
        ...
        .ok_or_else(|| "B Connect candidate was absent from the post-Connect census".to_owned())?;
    ...
    let stale_a_after_return = repository_stage_action(state.clone(), old_a_stage.clone()).await;
    if state
        .inner()
        .workspace_membership
        .lock()
...
async fn task_335_windows_host_driven_remembered_workspace_live_certification() -> Result<(), String>
{
    ...
    let p1_member = {
        ...
        let transcript = serde_json::to_string(&state.persistence.lock().unwrap().presentation())
    ...
    for (name, bytes, expected_status) in [
        ...
        let presentation = remembered_catalog_presentation(state.remembered_workspace.snapshot());
...
impl GetFileHash {
    fn sha256(path: &Path) -> Result<String, String> {
        let bytes = fs::read(path).map_err(|error| format!("Codex hash read failed: {error}"))?;
...
fn task352_repository_capture(git: &Path, root: &Path) -> Result<Task352RepositoryCapture, String> {
...
impl Task352LiveFixture {
    ...
    fn initialize_repository(&self, git: &Path, root: &Path, label: &str) -> Result<(), String> {
        task_324_git(git, root, &["init", "--quiet"])?;
        task_324_git(
            git,
            root,
...
async fn task_352_windows_active_repository_close_live_certification() -> Result<(), String> {
    ...
    let git = selected_git_executable().map_err(|_| "native Git discovery failed".to_owned())?;
    ...
    let (
        ...
        let catalog_presentation_before = serde_json::to_value(remembered_catalog_presentation(
            state.remembered_workspace.snapshot(),
        ))
        ...
        let index_before_commit_review =
            task352_repository_capture(&git, &fixture.repository_a)?.index_sha256;
        let commit_control = authorize_test_commit(&state, Arc::clone(&repository_before)).await;
        ...
        task352_require(
            !close_json.contains(private_sentinel)
                ...
                && close_result.membership.membership_generation == membership_generation_before
        ...
        task352_require(
            repository_index_action(&state, old_stage_selector, RepositoryIndexActionKind::Stage)
                .await
                .is_err()
                && repository_index_action(
                    &state,
        ...
        activate_admitted_member(&state, member_a)
            .await
            .map_err(|_| "production reactivation of A for connected Close failed".to_owned())?;
        ...
        let connected_runtime_claim = match connect_result {
            Ok(result) => {
                ...
                let Some((connected_runtime, connected_source)) = connected_state else {
                    ...
                    return Err(
                        "explicit Codex Connect returned without a connected runtime".to_owned(),
                    );
                };
                ...
                let connected_close_kept_generation = *state
                    .repository_generation
                    .lock()
                    .map_err(|_| "connected-case generation lock was poisoned".to_owned())?
                ...
                match &connected_process_ids {
                    Ok(process_ids) => {
                        for process_id in process_ids {
                            if task_324_wait_for_codex_process_exit(&codex_executable, *process_id)
                                .await
                                .is_err()
                            {
                ...
                let post_disconnect_kept_member = state
                    ...
                    .active_member()
                    == Some(member_a);
                let post_disconnect_kept_generation = *state
                    .repository_generation
                    .lock()
                    .map_err(|_| "post-disconnect generation lock was poisoned".to_owned())?
                    == generation_connected_a;
                println!(
            ...
            Err(FrontendError::CodexConnectionFailed) => {
                ...
                task352_require(
                    no_started_runtime
                        && state
                            .provider_activation
                            .lock()
                            .map_err(|_| "failed-connect provider lock was poisoned".to_owned())?
                            .is_none()
                        && state
                            .workspace_membership
                            .lock()
                            .map_err(|_| "failed-connect membership lock was poisoned".to_owned())?
                            .active_member()
                            == Some(member_a)
                        && *state.repository_generation.lock().map_err(|_| {
                ...
                disconnect_codex(app.state())
                    .await
                    .map_err(|_| "post-failure explicit disconnect lifecycle failed".to_owned())?;
                ...
                .map_err(|_| "Close after failed Codex connection did not succeed".to_owned())?;
                "not_executed_connection_failed"
            }
            Err(_) => {
                let _ = disconnect_codex(app.state()).await;
                return Err(
                    "certified Codex connection failed outside the bounded runtime case".to_owned(),
                );
            }
        };
...
impl Task361LiveFixture {
    ...
    fn cleanup(mut self) -> Result<Vec<u32>, String> {
        ...
        let pids = self
            ...
            .map_err(|_| "Task 361 child ownership record was poisoned".to_owned())?
            .clone();
        fs::remove_dir_all(&root).map_err(|_| "Task 361 fixture root cleanup failed".to_owned())?;
        self.cleaned = true;
        if self.root.exists() {
...
async fn task361_run_live_scenarios(fixture: &Task361LiveFixture) -> Result<(), String> {
    ...
    let fixture_root = fs::canonicalize(&fixture.root).map_err(|_| "fixture root unavailable")?;
    task361_require(
        !fixture_root.starts_with(&checkout),
    ...
    task361_require(
        std::path::Path::new(
            std::str::from_utf8(&capture_a.gitfile[8..capture_a.gitfile.len().saturating_sub(1)])
    ...
    fs::create_dir(&fabricated_root).map_err(|_| "fabricated gitfile candidate setup failed")?;
    fs::write(
    ...
    task361_require(
        a_observation.path == capture_a.root.display().to_string()
            && state.workspace_membership.lock().unwrap().active_member() == Some(fresh_member_a)
            && a_observation
                .staged_diff
                .iter()
                .any(|entry| entry.new_path.as_deref() == Some("a-initial-staged.txt"))
    ...
    let before_content_main =
        fs::read(fixture.main.join("edit.txt")).map_err(|_| "main content baseline unavailable")?;
    let before_content_b = fs::read(fixture.linked_b.join("edit.txt"))
        .map_err(|_| "B content baseline unavailable")?;
    let patch_preimage =
        fs::read(fixture.linked_a.join("edit.txt")).map_err(|_| "A patch preimage unavailable")?;
    ...
    task361_require(
        task361_output_status(&patch_output).as_deref() == Some("ok")
            ...
            && fs::read(fixture.main.join("edit.txt")).map_err(|_| "main file reread failed")?
                == before_content_main
            && fs::read(fixture.linked_b.join("edit.txt")).map_err(|_| "B file reread failed")?
                == before_content_b
    ...
    task361_require(
        task361_output_status(&commit_output).as_deref() == Some("committed_verified")
            ...
            && task361_capture(fixture, &fixture.linked_b)?.head == b_after_external_commit.head
    ...
    let stale_commit_registry = desktop_tool_registry(Some(&active_a), Some(current_commit_tool))
    ...
    task361_require(
        stale_review_authorization.is_err()
            ...
            && task361_capture(fixture, &fixture.linked_a)?.head == a_head_after_stale_rejection
    ...
    let index_before_final_close = [
        ...
        fs::read(&capture_a.index_path).map_err(|_| "A index before final Close unavailable")?,
        fs::read(&capture_b.index_path).map_err(|_| "B index before final Close unavailable")?,
    ];
    state
        .host_invocation
        .lock()
...
async fn task361_run_layout_and_external_identity_cases(
    ...
    task361_require_rejected_admission(fixture, "selected-root-reparse", &root_junction).await?;
    ...
    let private_reparse_result =
        task361_require_rejected_admission(fixture, "private-gitdir-reparse", &fixture.linked_a)
            .await;
    fs::write(&root_gitfile, original_gitfile).map_err(|_| "A gitfile cleanup restore failed")?;
    ...
    let common_reparse_result =
        task361_require_rejected_admission(fixture, "common-gitdir-reparse", &fixture.main).await;
    ...
    let worktrees_reparse_result =
        task361_require_rejected_admission(fixture, "worktrees-parent-reparse", &fixture.linked_a)
            .await;
    ...
    let metadata_member =
        admit_repository_with_semantic_validation(&metadata_state, &fixture.git, &metadata_root)
            .await
...
async fn task369_windows_repository_search_live_certification() -> Result<(), String> {
    ...
    let git =
        selected_git_executable().map_err(|_| "Task 369 native Git discovery failed".to_owned())?;
    ...
    task361_require(
        search_entry.effect_class == crate::effective_authority::EffectClass::ReadOnly
            ...
            && search_entry.source_kind == crate::effective_authority::SourceKind::RepositoryHost,
    ...
    for (query, expected_path) in [
        ...
        task361_require(
            value["matches"]
                .as_array()
                .is_some_and(|matches| matches.iter().any(|item| item["path"] == expected_path)),
...
async fn task379_windows_repository_list_live_certification() -> Result<(), String> {
    ...
    let git =
        selected_git_executable().map_err(|_| "Task 379 native Git discovery failed".to_owned())?;
    let fixture = Task361LiveFixture::new(git)?;
    let storage = TestRepository::new();
    let state = DesktopAppState::new(storage.0.clone());
    ...
    .map_err(|error| error.to_string())?;
    fs::create_dir_all(fixture.linked_a.join("ignored-only")).map_err(|error| error.to_string())?;
    fs::write(
    ...
    fs::create_dir_all(fixture.linked_a.join("physical-untracked"))
        .map_err(|error| error.to_string())?;
    fs::create_dir_all(fixture.linked_a.join("saturation")).map_err(|error| error.to_string())?;
    for index in 0..130 {
        fs::write(
            fixture
                .linked_a
    ...
    .map_err(|error| error.to_string())?;
    fixture.git_run(&fixture.linked_a, &["add", "--", "staged-new.txt"])?;
    fs::remove_dir_all(fixture.linked_a.join("deleted-only")).map_err(|error| error.to_string())?;
    fs::write(
    ...
    fs::remove_dir_all(fixture.linked_a.join("nested-case")).map_err(|error| error.to_string())?;
...
async fn task_361_windows_linked_worktree_live_certification() -> Result<(), String> {
    ...
    let git =
        selected_git_executable().map_err(|_| "Task 361 native Git discovery failed".to_owned())?;
    let fixture = Task361LiveFixture::new(git)?;
